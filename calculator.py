...
# class Car:
#     def __init__(self,
#                  name:str,price:int,fuel_economy:float,service_cost:int,insurance_cost:int):
#         self.name=name
#         self.price=price
...
#     def dymanic_year_cost(self,mileage:int):
#         return self.fuel_economy*mileage/100*get_gas_price()
#     def year_cost(self,mileage):
#         return self.static_year_cost()+self.dymanic_year_cost(mileage)
...
class Calculator:
    ...
    def add_car(self, car):
        year_cost = car.year_cost(self.mileage)  # Now passes mileage correctly
        price_per_year = car.price / self.years
        left_price = self.get_left_price(car) / self.years
        self.cars[car.name] = year_cost + price_per_year - left_price
    ...
    def print_cars(self):
        for car_name, year_price in self.cars.items():
            print(f'{car_name}: {year_price:.2f}')
